    ...
    in_outcome_distribution)
{
  ...
  {
      reg <-
          pop_01 %>% MASS::glm.nb(outcome_obs ~ treatment, data = ., maxit = 1000)
  }
  ...
  ## but the p-value is a test with power against the alternative that the treatment group is *higher* than the control group (ranks bigger on avearage). It estiamte the average difference in ranks.
...
Test the function
```{r}
test_output <- simulation_01(in_iteration = 1,
              in_population_size = 1112,
              in_ate=params$ate,
              in_population_mean=8,
              in_prop_assigned_treatment = .05,
              in_outcome_distribution = "normal")

test_output$p.value[2]
```
...
sims <-
  ...
  map_dfr(~simulation_01(in_iteration = .,
              in_population_size = 1112, # population_size_color_contrast_issues,
              in_ate = params$ate,
              in_population_mean = 8 , #population_mean_color_contrast_issues,
              in_prop_assigned_treatment = .05, #population_treatement_proportion,
              in_outcome_distribution = "normal")
  )
```

How often do we reject the null of no effects given the true ate of `r params$ate`:

```{r}
sims_trt <- sims %>% filter(term=="treatment") %>% dplyr::select(p.value)
summary(sims_trt)
## This is the power to detect an ate of 1 given significance level of .05 and other design parameters.
mean(sims_trt <= .05)
```

Which ate just pushes us over .8 for sims_trt above? That is the minimally detectable effect.
